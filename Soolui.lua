...
local Themes = {
    ["Preset"] = {
        ...
        ["Inline"] = FromRGB(22, 30, 12),
        ["Hovered Element"] = FromRGB(50, 68, 22),
        ["Page Background"] = FromRGB(26, 38, 14),
        ["Outline"] = FromRGB(58, 82, 25),
        ["Element"] = FromRGB(38, 54, 18),
        ["Gradient"] = FromRGB(208, 208, 208),
        ["Text"] = FromRGB(235, 235, 235),
        ["Text Stroke"] = FromRGB(0, 0, 0),
        ["Placeholder Text"] = FromRGB(185, 185, 185),
        ["Accent"] = FromRGB(180, 255, 0)
    }
}
...
-- == 对外接口（用户只能调这些）
-- ============================================================
local _Window = Library:Window({
    Logo = "77218680285262",
    FadeTime = 0.3,
})

local _MainPage = _Window:Page({Name = "主控"})
local _SecL = _MainPage:Section({Name = "功能", Side = 1})
local _SecR = _MainPage:Section({Name = "快捷键", Side = 2})

local _count = 0
local function _nextFlag()
    _count = _count + 1
    return "user_" .. _count
end
...
function API.AddToggle(name, callback)
    return _SecL:Toggle({
        Name = name, Flag = _nextFlag(), Default = false,
...
function API.AddSlider(name, min, max, default, callback)
    return _SecL:Slider({
        Name = name, Flag = _nextFlag(),
        Min = min, Max = max, Default = default,
        Decimals = 1, Callback = callback or function() end
    })
end

function API.AddButton(name, callback)
    return _SecL:Button():Add(name, callback or function() end)
end

function API.AddDropdown(name, items, default, callback)
    return _SecL:Dropdown({
        Name = name, Flag = _nextFlag(),
...
function API.AddTextbox(name, placeholder, default, callback)
    return _SecL:Textbox({
        Name = name, Flag = _nextFlag(),
...
function API.AddKeybind(name, defaultKey, callback)
    local lbl = _SecR:Label(name)
    return lbl:Keybind({
        Name = name, Flag = _nextFlag(),
        Default = defaultKey or Enum.KeyCode.F,
        Mode = "Toggle",
        Callback = callback or function() end
    })
end

function API.Notify(title, content, duration)
    Library:Notification(title, content, duration or 3)
end
